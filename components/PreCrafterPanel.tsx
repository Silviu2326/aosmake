...
import { Play, Copy, GitBranch, Search, Maximize2, Plus, Save, RotateCcw, Bot, Upload, Download, Layout, History, BoxSelect, Snowflake, Gift, Bell, Star, PartyPopper, Music, Heart, Zap, Cloud, Moon, DollarSign, Coins, Wallet, TrendingUp, Diamond, Briefcase, Dna, Trash2 } from 'lucide-react';
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        onClick={() => {
                            ...
                            if (!isChristmasMode) setIsMoneyMode(false);
                        }}
                        className={`flex items-center gap-1.5 px-2 py-1.5 rounded transition-colors ${isChristmasMode ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'hover:bg-white/10 text-gray-400 hover:text-white'}`}
                        title="Modo Navidad"
                    >
                    ...
                    <button
                        onClick={() => {
                            setIsMoneyMode(!isMoneyMode);
                            if (!isMoneyMode) setIsChristmasMode(false);
                        }}
                        className={`flex items-center gap-1.5 px-2 py-1.5 rounded transition-colors ${isMoneyMode ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'hover:bg-white/10 text-gray-400 hover:text-white'}`}
                        title="Modo Billetes"
                    >
                        <span className="text-xs">🤑 Billetes</span>
                    </button>

                    <button
                        onClick={() => setIsMutationModalOpen(true)}
                        className={`flex items-center gap-1.5 px-2 py-1.5 rounded transition-colors ${isMutationModalOpen ? 'bg-purple-500/20 text-purple-400 border border-purple-500/30' : 'hover:bg-white/10 text-gray-400 hover:text-white'}`}
                        title="Bio-Lab: Evolutionary Prompt Engine"
                    >
                        <Dna size={14} />
                        <span className="text-xs">Bio-Lab</span>
                    </button>
