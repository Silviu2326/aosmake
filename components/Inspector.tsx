...
import { X, Play, Save, ChevronRight, FileOutput, Shield, FlaskConical, Plus, Code, Trash2, GitBranch, Copy, CheckCircle2, Globe, Sparkles, Maximize2, Download, Dna, AlignLeft } from 'lucide-react';
...
export const Inspector: React.FC<InspectorProps> = ({ node, availableNodes = [], edges = [], onClose, onNodeUpdate, onDeleteNode }) => {
    ...
    const handleMutation = async (parentAId: string, parentBId: string | null, instructions: string) => {
        // 1. Resolve Parents Data
        const resolveData = (id: string) => {
            if (id === 'master') return { system: node.systemPrompt, user: node.userPrompt };
            const v = variants.find(v => v.id === id);
            return v ? { system: v.systemPrompt, user: v.userPrompt } : null;
        };

        const pA = resolveData(parentAId);
        const pB = parentBId ? resolveData(parentBId) : null;

        if (!pA) return;

        // 2. Construct Meta-Prompt for the LLM
        const metaSystemPrompt = `You are an expert Prompt Engineer specializing in prompt optimization and genetic algorithms.
Your goal is to create a NEW, superior System Prompt and User Prompt based on the provided parent(s) and instructions.

RETURN JSON ONLY:
{
  "label": "Short name for this variant (e.g. 'Merged V1+V2' or 'Mutated V1')",
  "systemPrompt": "The new full system prompt",
  "userPrompt": "The new full user prompt"
}`;

        let metaUserPrompt = `PARENT A:\n--- SYSTEM ---\n${pA.system}\n--- USER ---\n${pA.user}\n\n`;

        if (pB) {
            metaUserPrompt += `PARENT B:\n--- SYSTEM ---\n${pB.system}\n--- USER ---\n${pB.user}\n\n`;
            metaUserPrompt += `TASK: Merge Parent A and Parent B. ${instructions || 'Combine the best traits of both.'}`;
        } else {
            metaUserPrompt += `TASK: Mutate Parent A. ${instructions || 'Improve it.'}`;
        }

        // 3. Call Generic Run-Node Endpoint (Client-side Orchestration)
        try {
            const response = await fetch('https://backendaos-production.up.railway.app/api/workflows/run-node', {
                ...
                body: JSON.stringify({
                    node: {
                        type: 'LLM',
                        model: 'gemini-3-pro-preview', // Strong model for reasoning
                        ...
                        schema: JSON.stringify({
                            ...
                            required: ["label", "systemPrompt", "userPrompt"]
                        })
                    },
                    context: {}
                })
            ...
            if (data.success && data.output) {
                let result;
                try {
                    result = typeof data.output === 'string' ? JSON.parse(data.output) : data.output;
                } catch (e) {
                    console.error("Failed to parse mutation result", data.output);
                    return;
                }
                ...
                const newVariant: NodeVariant = {
                    id: `v_mut_${Date.now()}`,
                    label: result.label || 'Mutated Variant',
                    systemPrompt: result.systemPrompt,
                    userPrompt: result.userPrompt,
                    temperature: temperature // Inherit current temp
                };

                onNodeUpdate(node.id, {
                    variants: [...variants, newVariant],
                    activeVariantId: newVariant.id // Switch to it
                });
            }
        } catch (error) {
        ...
        try {
            const payload = {
                node: {
                    ...
                    userPrompt,
                    temperature,
                    schema: outputMode === 'structured' ? jsonSchema : undefined,
                    outputMode
                },
                debug: {
                    modelState: model,
    ...
    const handleOutputModeChange = (mode: 'structured' | 'free') => {
        setOutputMode(mode);
        updateActiveData({ outputMode: mode });
    };
            ...
            <div className="h-12 flex items-center justify-between px-4 border-b border-border bg-surfaceHighlight">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="flex items-center gap-1 px-2 py-1 rounded bg-green-500/10 hover:bg-green-500/20 text-green-400 hover:text-green-300 text-xs transition-colors border border-green-500/20"
                        title="Mutate or Merge Variants"
                    >
                        <Dna size={12} /> Lab
                    </button>
                ...
                {activeTab === 'prompt' && (
                    <div className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <div className="h-32 border border-border rounded bg-background focus-within:border-accent relative">
                                <VariableHighlighter
                                        ...
                                        return (
                                            <div key={n.id} className="mb-2 last:mb-0">
                                                ...
                                                <div className="flex flex-wrap gap-1">
                                                    ...
                                                    <button
                                                        onClick={() => handleAddVariable(`${n.id}.output`)}
                                                        className="px-2 py-0.5 rounded bg-black/40 border border-white/10 text-[10px] text-purple-400 hover:border-purple-500/50 hover:bg-purple-500/10 transition-all flex items-center gap-1 opacity-80 hover:opacity-100"
                                                        title="Insert complete JSON output"
                                                    >
                                                        <Plus size={8} /> output (raw)
                                                    </button>
                                                </div>
                                            </div>
                                        );
                        ...
                        <div className="space-y-2 pt-2 border-t border-white/5">
                            <label className="text-xs text-gray-500 font-semibold block">OUTPUT MODE</label>
                            <div className="flex bg-black/20 p-1 rounded border border-white/10">
                                <button
                                    onClick={() => handleOutputModeChange('structured')}
                                    className={`flex-1 flex items-center justify-center gap-2 py-1.5 rounded text-xs transition-all ${outputMode === 'structured' ? 'bg-accent/20 text-accent border border-accent/20' : 'text-gray-500 hover:text-gray-300'}`}
                                >
                                    <Code size={12} />
                                    Structured JSON
                                </button>
                                <button
                                    onClick={() => handleOutputModeChange('free')}
                                    className={`flex-1 flex items-center justify-center gap-2 py-1.5 rounded text-xs transition-all ${outputMode === 'free' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/20' : 'text-gray-500 hover:text-gray-300'}`}
                                >
                                    <AlignLeft size={12} />
                                    Any JSON
                                </button>
                            ...
                            {outputMode === 'free' && (
                                <div className="text-[10px] text-gray-500 italic px-1">
                                    The model will be forced to output valid JSON, but the structure is not strictly defined.
                                </div>
                            )}
                        </div>
